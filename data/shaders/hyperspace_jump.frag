...
vec3 pixelVal(vec2 coord) {
    ...
    if(time < 3.5) {
        ...
        float whiteIntensity = smoothstep(0.7, 1.0, noiseVal) * (1.0 - smoothstep(2.8, 3.5, time));
    ...
    if(time >= 2.8 && time < 4.5) {
        ...
        float r = ad.y / (1.0 + 0.15 * linearstep(0.0, 3.5, time)) * (40.0 / (3.0 + 20.0 * jumpstep(2.8, 6.0, 0.5 * pow(time, 1.5))));
        float noiseVal = simplexNoise(vec3(40.0 * ad.x, r, 0.0));
        col = mix(col, vec3(0.5, 0.7, 1), smoothstep(2.8, 3.5, time) * smoothstep(0.4 * blueTime, blueTime, noiseVal));
        col = mix(col, vec3(1, 1, 1), smoothstep(2.8, 3.5, time) * smoothstep(0.7 * whiteTime, whiteTime, noiseVal));
