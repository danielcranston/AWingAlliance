...
uniform float hyperspace_scale;
...
float Noise(in vec2 p, in float scale)
{
    ...
    float res = mix(mix(Hash(p, scale), Hash(p + vec2(1.0, 0.0), scale), f.x),
                    mix(Hash(p + vec2(0.0, 1.0), scale), Hash(p + vec2(1.0, 1.0), scale), f.x),
                    f.y);
    return res;
}
...
float fBm(in vec2 p)
{
    ...
    float scale = hyperspace_scale;  // 40.0;
    p = mod(p, scale);
    float amp = 0.6;
...
void main(void)
{
    ...
    // out_Color = vec4(0.0, gl_FragCoord.y / resolution.y, 0.0, 1.0);
}
